...
const IR_PRESENT = ["voy", "vas", "va", "vamos", "vais", "van"] as const;
...
function regularPresent(verb: string): string[] {
  if (verb.endsWith("ar")) {
...
function cycleToLength<T>(items: T[] | undefined, target: number): T[] {
  if (!items?.length) {
    return [];
  }
...
function isInformalFutureUnit(unit: Lesson): boolean {
  const unitLabel = unit.unit?.toLowerCase() ?? "";
  const patternLabel = unit.pattern?.toLowerCase() ?? "";
  return unitLabel.includes("informal future") || patternLabel.includes("ir (present) + a + infinitive");
}

function buildGeneratedDrills(verbs: string[], informalFuture: boolean) {
  const conjugation: { prompt: string; answer: string }[] = [];
  const recognition: { form: string; meaning: string }[] = [];
  const sentenceBuilding: { prompt: string; answer: string }[] = [];

  PRONOUNS.forEach((pronoun, personIndex) => {
    verbs.forEach((verb, verbIndex) => {
      const presentForms = conjugatePresent(verb);
      const answer = informalFuture ? `${IR_PRESENT[personIndex]} a ${verb}` : presentForms[personIndex];
      const recognitionMeaning = informalFuture ? `${pronoun} is going to ${verb}` : `${pronoun} ${verb}`;

      conjugation.push({ prompt: `${pronoun} / ${verb}`, answer });
      recognition.push({ form: answer, meaning: recognitionMeaning });
      ...
      const englishAction = informalFuture ? `is going to ${verb}` : verb;
      const english = `${subject.english} ${englishAction} ${object.english} ${context.english}`;
...
export function expandUnitDrills(unit: Lesson): Lesson {
  const verbs = [...(unit.verbs ?? [])];
  const generated = buildGeneratedDrills(verbs, isInformalFutureUnit(unit));
