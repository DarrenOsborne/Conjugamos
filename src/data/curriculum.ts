import { expandUnitDrills } from "./drillGenerator";
import { validateLesson, type CurriculumUnit } from "./schema";
...
export const curriculum: CurriculumUnit[] = lessons.map((lesson, index) => {
  const lessonNumber = index + 1;
  const validated = validateLesson(lesson, `lesson${String(lessonNumber).padStart(2, "0")}`);
